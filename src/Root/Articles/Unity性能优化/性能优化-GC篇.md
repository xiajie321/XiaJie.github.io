...
| 特性       | 栈 (Stack)                                               | 托管堆 (Managed Heap)           |
|:-------- |:------------------------------------------------------- |:---------------------------- |
| **形象比喻** | 随身携带的**背包**                                             | 远处的**大仓库**                   |
| **存储内容** | 值类型 (`int`, `struct`)、引用类型的**地址**                       | 引用类型的**实例** (`class object`) |
| **存取速度** | 极快 (LIFO)                                               | 较慢 (需要寻址)                    |
| **管理方式** | 自动管理 (作用域结束即销毁)                                         | **GC 管理** (垃圾回收器负责)          |
| **示例**   | `int a = 10;`<br>`Vector3 pos;`<br>`ClassA ref;` (只是地址) | `new ClassA();` (实际对象)       |

> **引用关系说明**：
> 如上表所示，引用类型的变量（如 `ClassA ref`）本身是在**栈**上的，它存储的只是一个**地址**（就像一张提货单）。而实际的对象数据（`new ClassA()`）是在**堆**上的。
> 当你访问 `ref.data` 时，实际上是拿着栈上的地址，去堆里找到对应的数据。
> 
> **比喻说明**：
> 
> * **栈**就像你的**背包**，`int` 和 `struct` 是直接装在背包里的现金和工具，取用非常快。
> * **堆**就像远处的**仓库**，`class` 对象（如大电视）太大装不进背包，只能放在仓库里。
> * **引用**就是你背包里的一张**提货单**（地址），上面写着电视机在仓库的哪个位置。
